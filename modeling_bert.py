...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self):
        ...
        self.token_type_embeddings = nn.Embedding(30, 768)
...
class BertLMPredictionHead(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Linear(768,
                                 28996,
                                 bias=False)

        self.bias = nn.Parameter(torch.zeros(28996))
...
class BertOnlyNSPHead(nn.Module):
    def __init__(self):
        ...
        self.seq_relationship = nn.Linear(768, 2)
...
class BertForPreTraining(BertPreTrainedModel):
    r"""
        **masked_lm_labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size, sequence_length)``:
            ...
            Indices should be in ``[-1, 0, ..., 28996]`` (see ``input_ids`` docstring)
    ...
    Outputs: `Tuple` comprising various elements depending on the configuration (config) and inputs:
        ...
        **prediction_scores**: ``torch.FloatTensor`` of shape ``(batch_size, sequence_length, 28996)``
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        ...
        if masked_lm_labels is not None and next_sentence_label is not None:
            ...
            masked_lm_loss = loss_fct(prediction_scores.view(-1, 28996), masked_lm_labels.view(-1))
...
class BertForSequenceClassification(BertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            ...
            Indices should be in ``[0, ..., 3 - 1]``.
            If ``3 == 1`` a regression loss is computed (Mean-Square loss),
            If ``3 > 1`` a classification loss is computed (Cross-Entropy).

    Outputs: `Tuple` comprising various elements depending on the configuration (config) and inputs:
        **loss**: (`optional`, returned when ``labels`` is provided) ``torch.FloatTensor`` of shape ``(1,)``:
            Classification (or regression if 3==1) loss.
        **logits**: ``torch.FloatTensor`` of shape ``(batch_size, 3)``
            Classification (or regression if 3==1) scores (before SoftMax).
        **hidden_states**: (`optional`, returned when ``config.output_hidden_states=True``)
            list of ``torch.FloatTensor`` (one for the output of each layer + the output of the embeddings)
            of shape ``(batch_size, sequence_length, hidden_size)``:
            Hidden-states of the model at the output of each layer plus the initial embedding outputs.
